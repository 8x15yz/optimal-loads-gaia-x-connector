...
"참여 세션 연결 (운영자 설정 적용)": "Participant session connected (configured duration)",
"참여 세션 시간 변경": "Participant session duration changed",
...
Object.assign(UI_MESSAGES,{"ix_renew": {"en": ["After expiry, reconnect your participant session and establish the service contract again."], "ko": ["만료 후 참여 세션을 다시 연결하고 서비스 계약을 다시 체결해주세요."]}});
Object.assign(UI_MESSAGES,{"ss_title": {"en": ["Participant session duration"], "ko": ["참여 세션 유효시간"]}, "ss_new": {"en": ["New sessions only"], "ko": ["새 세션부터 적용"]}, "ss_hint": {"en": ["Set how long a participant can remain connected after credential verification."], "ko": ["자격증명 검증 후 참여 세션을 유지할 시간을 설정합니다."]}, "ss_label": {"en": ["Duration in seconds · 1–604800"], "ko": ["유효시간(초) · 1~604800"]}, "ss_save": {"en": ["Save duration"], "ko": ["시간 저장"]}, "ss_examples": {"en": ["60 = 1 minute · 3600 = 1 hour · 86400 = 24 hours (default)"], "ko": ["60초 = 1분 · 3600초 = 1시간 · 86400초 = 24시간(기본값)"]}, "ss_scope": {"en": ["Applies to all newly connected participant sessions. Existing sessions, contracts, access keys and account sign-in sessions stay unchanged. Credential validity may shorten the duration. Settings persist after restart."], "ko": ["전체 계정의 새 참여 세션부터 적용됩니다. 기존 세션·계약·접속 키와 로그인 세션은 변경되지 않습니다. 자격증명 등의 유효기한이 더 짧으면 해당 기한까지만 유지됩니다. 설정은 재시작 후에도 유지됩니다."]}, "ss_invalid": {"en": ["Enter a whole number from 1 to 604800."], "ko": ["1~604800 사이의 정수 초를 입력하세요."]}, "ss_saved": {"en": ["Saved. Applies from the next participant connection."], "ko": ["저장했습니다. 다음 참여 세션 연결부터 적용됩니다."]}});
let uiLocale='en';
