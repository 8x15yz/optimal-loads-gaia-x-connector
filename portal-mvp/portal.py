"""Local-only FastAPI demo. External credentials are imported, never issued here."""
import argparse
import hashlib
import json
import os
...
def register(req: ServiceRequest, x_portal_local: str = Header('')):
    local_header(x_portal_local)
...
def services(x_portal_local: str = Header('')):
    local_header(x_portal_local)
    with db() as c:
...
def negotiate(req: NegotiateRequest, authorization: str = Header('')):
    ...
    with db() as c:
        service = c.execute('SELECT * FROM services WHERE id=?', (req.service_offering_id,)).fetchone()
        if not service:
        ...
        ident = str(uuid.uuid4())
        expires = min(session['expires'], time.time() + 900)
        c.execute('INSERT INTO contracts VALUES (?,?,?,?,?)', (ident, session['participant_id'], service['id'], service['country'], expires))
    return {'contract_id': ident, 'expires_at': expires, 'participant_id': session['participant_id'],
            'mode': 'demo-access-grant', 'notice': '15분 데모 접근 승인입니다. 양측 서명 DAC/DUA는 아직 구현하지 않았습니다.'}
...
def transfer(req: TransferRequest, authorization: str = Header('')):
    ...
    with db() as c:
        ...
        if not contract or contract['participant_id'] != session['participant_id']:
